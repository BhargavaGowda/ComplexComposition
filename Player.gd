...
func _process_input(delta):
	...
	if Input.is_action_pressed("shoot"):
		...
		var result = space_state.intersect_ray(get_node("Collider/Camera").global_translation, get_node("Collider/Camera").to_global(Vector3(0,0,-50)), [self])
		if not result.empty():
			var body = result.collider
			if body.get("colorNeuron"):
				body.colorNeuron.input = -1
#				for i in range(len(body.neurons)):
#					body.neurons[i].a*=0.1
#					body.neurons[i].cached_a*=0.1
##					for j in range(len(body.neurons[i].inpWeights)):
##						body.neurons[i].inpWeights[j]*=0.99
